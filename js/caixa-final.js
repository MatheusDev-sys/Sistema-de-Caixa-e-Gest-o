...
async function atualizarTotais() {
    try {
        ...
        const totalVendas = vendas.reduce((sum, v) => sum + parseFloat(v.valor), 0);
        const totalVendasElement = document.getElementById('caixaTotalVendas');
        if (totalVendasElement) {
            totalVendasElement.textContent = utils.formatarMoeda(totalVendas);
        }
...
// Renderizar lista de vendas com botões de editar
function renderizarVendas() {
    const container = document.getElementById('vendasList');

    if (!container) {
        console.warn('Container vendasList não encontrado');
        return;
    }
    ...
    container.innerHTML = vendas.map(venda => `
        <div class="flex justify-between items-center p-3 rounded-lg bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 transition-colors">
            <div class="flex items-center gap-3 flex-1">
                ...
                <div class="flex-1">
                    <p class="text-gray-800 dark:text-gray-100 font-medium">${venda.quantidade}x ${security.sanitizeHTML(venda.descricao)}</p>
                    <p class="text-gray-500 dark:text-gray-400 text-sm">
                        ${formatarPagamento(venda.pagamento)} • ${utils.formatarDataHora(venda.criado_em)}
                        ${venda.observacao ? `<br><small>${security.sanitizeHTML(venda.observacao)}</small>` : ''}
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <p class="font-semibold text-gray-800 dark:text-gray-100">${utils.formatarMoeda(venda.valor)}</p>
                <button data-venda-id="${security.sanitizeAttribute(venda.id)}" class="btn-editar-venda p-2 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary transition-colors" title="Editar venda">
    ...
    document.querySelectorAll('.btn-editar-venda').forEach(btn => {
        btn.addEventListener('click', function () {
            const vendaId = this.getAttribute('data-venda-id'); // UUID como string
            console.log('🔧 Clicou em editar, ID:', vendaId);

            if (!vendaId) {
                console.error('❌ ID não encontrado no botão!');
                utils.mostrarNotificacao('Erro: ID da venda não encontrado', 'error');
                return;
            }
...
function abrirModalEditarVenda(vendaId) {
    console.log('🔍 Procurando venda ID:', vendaId, 'Tipo:', typeof vendaId);
    console.log('📋 Vendas disponíveis:', vendas.map(v => ({ id: v.id, tipo: typeof v.id })));

    // Comparar IDs como strings (UUIDs)
    const venda = vendas.find(v => String(v.id) === String(vendaId));
    if (!venda) {
        console.error('❌ Venda não encontrada! ID procurado:', vendaId);
        console.error('📋 IDs disponíveis:', vendas.map(v => v.id));
        utils.mostrarNotificacao('Venda não encontrada', 'error');
        return;
    }
    ...
    document.getElementById('editDescricaoVenda').value = venda.descricao;
    document.getElementById('editQuantidadeVenda').value = venda.quantidade;
    document.getElementById('editValorVenda').value = venda.valor;
    document.getElementById('editPagamentoVenda').value = venda.pagamento;
    document.getElementById('editObservacaoVenda').value = venda.observacao || '';
...
async function salvarEdicaoVenda(e) {
    ...
    try {
        ...
        const descricaoInput = document.getElementById('editDescricaoVenda').value;
        const quantidadeInput = document.getElementById('editQuantidadeVenda').value;
        const valorInput = document.getElementById('editValorVenda').value;
        const observacaoInput = document.getElementById('editObservacaoVenda').value;

        const dadosAtualizados = {
            descricao: security.validarDescricao(descricaoInput),
            quantidade: security.validarQuantidade(quantidadeInput),
            valor: security.validarValorMonetario(valorInput),
            pagamento: document.getElementById('editPagamentoVenda').value,
            observacao: security.validarObservacao(observacaoInput)
        };
        ...
        await utils.registrarAuditoria('EDITAR_VENDA', 'vendas', {
            ...
            campos_alterados: {
                antes: {
                    descricao: vendaOriginal.descricao,
                    quantidade: vendaOriginal.quantidade,
                    valor: vendaOriginal.valor,
                    pagamento: vendaOriginal.pagamento,
                    observacao: vendaOriginal.observacao
                },
                depois: dadosAtualizados
            }
        });
